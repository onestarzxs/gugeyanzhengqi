...
namespace MFAScreenLockApp
{
    public partial class FormQR : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if(string.IsNullOrWhiteSpace(textBox1.Text.Trim())||textBox1.Text.Trim().Length<=7) {
                MessageBox.Show("至少6个字符。\n", "创建失败", MessageBoxButtons.RetryCancel, MessageBoxIcon.Warning);
                return;
            }             
                string totpSecretKey = Guid.NewGuid().ToString("N").Substring(0, 10);
                var tfa = new TwoFactorAuthenticator();
            string computername = txt_code.Text.Trim();
                string username = textBox1.Text.Trim();
                var setupInfo = tfa.GenerateSetupCode(username, computername, totpSecretKey, false, 380);
           string otpauth = "otpauth://totp/" +"qifeipay.com" + ":" + setupInfo.Account + "?secret=" + setupInfo.ManualEntryKey + "&issuer=" + computername;
           
              lbl_EntryKey.Text = setupInfo.ManualEntryKey;
                 secretKey = totpSecretKey;
                //secretKey = textBox1.Text.Trim();
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            if (isinit) { 
                ...
                // 生成当前的谷歌验证码
                string currentCode = tfa.GetCurrentPIN(secretKey);

            // 更新显示验证码的标签控件
            lbl_codeisok.Text = "" + currentCode;
